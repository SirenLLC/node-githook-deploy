...
forever_template = """#Forever settings
...
try:
    ...
    use_forever = raw_input('Would you like to use forever? [y/n]' ).lower() == 'y'
    if use_forever:
        mode_var = raw_input('Enter node execution mode (Default production): ')
    ...
    with open(post_receive_path, 'wb') as post_recv_file:
        post_recv_file.write(post_receive_template.format(abs_path=abs_path))
        if use_forever:
            post_recv_file.write(forever_template.format(app_path=APP_PATH, env_vars=ENV_VARS))
